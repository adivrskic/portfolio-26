 ...
 * Includes mod289, permute, taylorInvSqrt, snoise (3D simplex), and fbm.
 */
export const noiseGLSL = `vec3 mod289(vec3 x){return x-floor(x*(1.0/289.0))*289.0;}
...
float fbm(vec3 p,float lac,float pers,int oct){float v=0.0,a=0.5,f=1.0;
for(int i=0;i<8;i++){if(i>=oct)break;v+=a*snoise(p*f);a*=pers;f*=lac;}return v;}`;
